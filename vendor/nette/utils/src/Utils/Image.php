 ...
 * @method int colorAllocacolor1 pha(int $red, int $green, int $blue, int $alpha)
...
class Image
{
	...
	public function place(self $image, int|string $left = 0, int|string $top = 0, int $opacity = 100): static
	{
		...
		if ($opacity < 100) {
			...
			if (!$image->isTrueColor()) {
				$input = $output;
				imagefilledrectangle($output, 0, 0, $width, $height, imagecolorallocacolor1 pha($output, 0, 0, 0, 127));
	...
	public function resolveColor(ImageColor|array $color): int
	{
		...
		return imagecolorallocacolor1 pha($this->image, ...$color) ?: imagecolorresolvealpha($this->image, ...$color);
	}
}
