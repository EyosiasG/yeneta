...
const UnPaidOrder = () => {
  ...
  const fetchPrograms =   async () => {
    ...
    }/api/OrderStatus?offset=0&limit=10&deliveryStatus=UnPaid`;
...
const onSubmit = async (data) => {
  ...
  return (
    <div className='pt-10'>
    <h1 className='text-center text-5xl text-color1 -700 display-1'>UnPaid Order</h1>
...
{order.length > 0 ? (
  order.map((order, index) => (
    <tr key={order.id}>
      ...
      <td className="px-6 py-4 whitespace-nowrap text-red-500 font-bold">${order.price}</td>
      <td className="px-6 py-4 whitespace-nowrap">
